...
| 项 | 说明 |
|---|---|
| ~~**真机连接**~~ | ✅ 已完成。Redmi K60 Ultra（corot）· Android 15 · `DYDIMNXWEYBAVSUG`。**验证时间：2026-09-18** |
...
| 4 | 身体数据与可视化 | ~12 | 能回答「三个月有没有变强」｜**4A ✅ · 4B ✅ · 4C 待做** |
...
> **拆成三块。** 需求文档里 M6（身体数据，5 个子模块）+ M7（10 张图）一次做完
> 是 Phase 3 量级的三四倍，而 **M7 的 10 张图里有 6 张只依赖已有的训练数据**
> ——两者可以独立交付。

| 块 | 内容 | 状态 |
|---|---|---|
| **4.0 环境欠账** | 测试库接线（`gym_log_test` 建了从没用）、组数口径五处收拢 | ✅ 完成 |
| **4.1 统计纯函数层** | `WeekSeries` / `MovingAverage` / `Adherence`（34 个测试） | ✅ 完成 |
| **4.2 统计聚合层** | `StatsMapper` / `StatsService` / `StatsController`（3 个端点） | ✅ 完成（188 测试全绿 + HTTP 端到端验证） |
| **4.3 造数据脚本** | `DemoDataSeeder`：12 周 × 3 练 + deload + 空周 + >14 天空档 | ✅ 完成 |
| **4.4 客户端图表页** | `fl_chart` + 「趋势」页 + 单动作历史页（按 M7-B 适配） | ✅ 完成（release 包真机验证 `AC-7B-1/2/3`） |
| **4B 身体数据** | `body_metric` 表 + 录入 + 体重/体脂/围度趋势图 | ✅ 完成（V15 + 14 个指标 + 4 个端点 + 趋势页 + 录入面板） |
| **4C 体态照片** | 上传 / 压缩 / 并排对比 | ⬜（**存储方案 Q2 未定，另行立项**） |

**4B 顺带修掉一个既有 bug**：`AC-7-8`（自重动作容量 = 体重 × `bw_factor` × 次数）
一直是坏的——展开函数不带 `bw_factor`，快照里永远是 NULL，而 0 正是「没记体重」的合法值，
把断链掩盖了几个月。详见 DEV-LOG 4B。

**4A（训练数据统计与图表）到此收口。** 客户端测试从 0 到 11 个用例
（`app/test/chart_axis_test.dart`），并在真机上用 **release 包**验证——
此前所有「真机验证」跑的都是 debug 包，这一轮暴露出主 manifest 缺 INTERNET 权限。

**4A 的图表**（只依赖已有训练数据，不需要新表）：
单动作 e1RM 曲线、周容量趋势、肌群周组数、训练频率 + streak、符合率、PR 看板。

**显式不做**（理由见 DEV-LOG 与 METRICS）：
出勤率（分母在当前 schema 下算不出来）、血压（二元组，窄表语义不成立）、
最多次数 / 最大容量两类 PR、主观状态相关性（仅 PC，归 Phase 5）、
图表离线缓存（依赖 3.12 的本地 SQLite）。

**你写**：聚合的正确性验收（造数据后查库手算一遍周容量，与接口返回比对）
...
| 3.3 | 组记录 CRUD | ✅ 完成 |
| 3.4 | 训练总结与历史列表 | ✅ 完成 |
| 3.5 | Flutter 骨架 + 网络层 + 最小登录闭环 | ✅ 完成 |
| 3.6 | **跟练页（顺序版）** ← 状态机核心 + 记录组 | ✅ 完成 |
| 3.7 | 训练总结页 | ✅ 完成 |
| 3.8 | 首页完善（断点续训入口 + 模板库） | ✅ 完成 |
| 3.10 | 超级组轮次推进 | ⬜ |
| 3.11 | 音频隔离（**建议提前到 3.7 之后先验**） | ✅ 完成（已按建议提前） |
| 3.12 | 本地 SQLite（drift）与断点续训 | ⬜ |
| 3.13 | 离线队列与幂等同步 | ⬜ |
| 3.14 | Phase 3 验收 | ⬜ |
| 3.15 | 时长类动作的组内计时（平板支撑倒计时 + 间隔播报 + `targetDurationSec` 字段） | ✅ 完成 |

> 3.9 这个编号空着没用（3.8 之后直接跳到 3.10）。
...
**Phase 4B —— 身体数据（`body_metric` 表 + 录入 + 体重/体脂/围度趋势图）。**

用户已明确选择「4A + 4B 一起做」。4A 已收口，4B 的设计要点见 DEV-LOG：

- **窄表单表** `body_metric(metric_type, site, value, measured_at, ...)`，
  `site` 按 `metric_type` 白名单校验（围度 12 个部位，与 `METRICS 2.1` 的 9 个冲突，
  取 `REQUIREMENTS M6-A-2` 的 12 个并改 `METRICS`）
- `DECIMAL(6,2)` 不是 `(5,2)`——BMR 约 1200–2500 kcal 会溢出
- **必须幂等**：这是离线优先架构下第一个没有幂等键的写接口，
  照 `set_record` 的做法上唯一键 + UPSERT
- **不做血压**：一次测量拆两行、靠 `measured_at` 完全相同配对，
  窄表的「一个指标一条序列」前提不成立
- 顺手填 `workout_session.body_weight_kg`（`TrainingMetrics.setVolume` 已经在读它，
  目前恒为 NULL）——**取 `measured_at <= startedAt` 的最近一条**，
  且只影响之后的会话，图上会出现一次台阶式跳变，**必须写进文档**

其它待办（未排期）：

| # | 待办 | 来源 |
|---|---|---|
| 1 | Phase 3 欠账：3.10 超级组轮次推进 / 3.12 本地 SQLite / 3.13 离线队列 / 3.14 验收 | Phase 3 |
| 2 | 手机端自定义计划编辑器（用户明确要做，是一个小型独立阶段） | 用户 |
| 3 | `M7-B-5` 图表离线缓存 | Phase 4A |
| ~~4~~ | ~~动作库扩充~~ | ✅ 4C 完成（125 个动作 + 浏览页） |
| 4 | 动作加「细分部位」字段（上胸/下背/股四/腘绳……）用于筛选，`MuscleGroup` 保持 6 个用于统计 | 同上（4C 已补了器械/模式两个筛选轴，细分部位仍未做） |
| 5 | 「个人模版」：用户把调好的计划存成模版复用 | 同上 |
| 6 | 人体肌肉热力图（替代/补充现在的 6 条横条） | 同上，需解剖图素材 |
| 7 | **每个指标加「环比上周」**，且「持平」作为明确状态 | 历史模块对照 |
| 8 | **部位之间的横向对比**（现在只有时间序列） | 同上 |
| 9 | 「管理部位 / 管理动作」：用户自选关注对象 | 同上 |
| 10 | **有氧与力量分开统计**（现在 `DURATION`/`DISTANCE_DURATION` 的动作混在 `LEGS` 里） | 同上 |
| 11 | 统计页板块可拖拽排序 | 同上 |
| 12 | **底部导航**：Today 页 AppBar 已挤到 4 个图标 | 4C |

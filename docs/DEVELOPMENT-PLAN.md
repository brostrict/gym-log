...
| 2.10 | 从模板创建计划 | ✅ 完成 |
| 2.11 | 逐组处方 | ✅ 完成（含在 2.8 的嵌套创建里） |
| 2.12 | 超级组编排 | ✅ 完成（含在 2.8 的嵌套创建里） |
| 2.13 | 周期化展开算法 | ✅ 完成 |
| 2.14 | 计划结构编辑（~~版本化~~，见下方说明） | ✅ 完成 |
| 2.15 | 「今天练什么」查询 | ✅ 完成 |
| 2.16 | Phase 2 验收 | ✅ 完成 |
| 2.17 | 补充动作要领与常见错误（41 个模板动作） | ✅ 完成 |

> 🎉 **Phase 2 完成（2026-09-18）**。43 项验收 42 通过，1 项是已知内容缺口。
> 交付：7 张新表、9 个迁移、90 个内置动作、6 个内置模板、71 个测试全绿。
> 详见 [DEV-LOG.md Phase 2 完成总结](./DEV-LOG.md#步骤-216--phase-2-验收-)。
>
> **验收抓出 3 个真问题**（2 个是我自己写的 API 缺陷）：
> 筛选参数写错会被静默忽略返回全量数据、`setIgnoreUnknownFields(false)` 会让请求头
> 变成非法属性导致全站 500、错误信息泄露内部类名。
> **如果没有这一步，它们会潜伏到写 Flutter 客户端时才爆出来。**

### Phase 2 欠账（带进 Phase 3）

| # | 欠账 | 影响 | 处理时机 |
|---|---|---|---|
| 1 | `SessionCounter` 恒返回 0 | 训练日轮转永远推荐第 1 个 | **Phase 3 第一件事** |
| 2 | ~~动作要领 / 常见错误（M2-4，Should）90 条全空~~ | — | ✅ 2.17 已补 41 个 |
| 3 | 删除计划时不检查是否有训练记录 | 历史记录指向已删除的计划 | Phase 3 |
| 4 | AC-3-1 / 3-2 / 3-3 未验证（依赖会话快照） | — | Phase 3 |

> **关于欠账 2**：只补了 6 个内置模板引用到的 41 个动作，其余 49 个冷门变式留空。
> 选择依据与覆盖率核对见 [DEV-LOG 步骤 2.17](./DEV-LOG.md#步骤-217--补充动作要领与常见错误-)。

> **2.13 / 2.14 的归属变更**：两步原本都标注「你写」，
> 但关键决策（`-40%` 的语义、返回四元组、全量替换 + 乐观锁）定下之后，
> 实现就变成了机械劳动，遂由 Claude 写完并逐行讲清。
> **如果你想自己写后面某一步**，说一声，Claude 把规格写详细后留给你。
>
> **2.14 从「版本化」降级为「结构编辑」**：原计划是「改结构 = 归档旧版 + 建新版本」，
> 后来发现**会话快照（Phase 3）已经覆盖了主要风险**——
> 训练记录冻结的是当时的处方副本，不是计划 id，所以改结构不会改写历史。
> 版本化只对「回滚到旧版」有价值，V1 不做。
> 详细论证见 REQUIREMENTS 4.2 与 6.3 不变量 2。
...
|---|---|---|
| 3.1 | 会话三张表与实体 | ✅ 完成 |
| 3.2 | 会话创建与快照（含 `SessionCounter` 真实实现） | ⬜ |
| 3.3 | 组记录 CRUD | ⬜ |
| 3.4 | 训练总结与历史列表 | ⬜ |
| 3.5 | 跟练状态机（Flutter） | ⬜ |
| 3.6 | 超级组轮次推进 | ⬜ |
| 3.7 | 本地 SQLite（drift）与断点续训 | ⬜ |
| 3.8 | 音频隔离 | ⬜ |
| 3.9 | 离线队列与幂等同步 | ⬜ |

> **顺序建议（从 Phase 2 概要沿用）**：先做**不带超级组的顺序执行**跑通，
> 再叠加超级组。一次性写完很难定位 bug。

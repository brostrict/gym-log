...
| **产出** | `JwtUtil`、`LoginRequest`、`AuthService.login`、`/auth/login` |
| **知识点** | JWT 三段结构（header.payload.signature）、HS256 签名、`sub`/`exp`/`iat` 标准声明、**为什么 JWT 不能存敏感信息**（payload 只是 Base64，不是加密） |
| **谁写** | **我写，你审查** —— 这是 mini_mall 已写过的标准套路，重复收益低 |
| **验收** | 登录返回 token；用 [jwt.io](https://jwt.io) 解码能看到正确 claims；密码错误返回统一错误码 |

**审查时你要能回答**：
- JWT 的三段分别是什么？payload 是加密的吗？
- 签名密钥从哪来？为什么不能硬编码？
- access token 和 refresh token 的过期时间该一样吗？为什么？

...
public class ExerciseQuery {
     ...
     *
     * <p><b>为什么从 100 提到 500</b>：动作库现在有 125 个内置动作，
     * 而「动作库浏览」这个场景**天然要一次看全**——用户是在找某个动作，
     * 不是在翻页。上限 100 会让第 101–125 个动作在界面上直接消失，
     * 而列表看起来完全正常（只是少了几个），不报任何错。
     *
     * <p>500 仍然是有界的：它挡的是「一次拉走整张表」，
     * 而不是「一次拉走一屏够用的数据」。表再长几个数量级时，
     * 该做的是客户端分页，而不是继续抬这个数。
     */
